...
- **2026-04-02**: 案件管理页面添加"见习签到表"功能（完善版）：
  - 年月选择弹窗改为日历切换样式（左右箭头切换月份，当前显示即选中）
  - 每个见习人员独立的日期范围选择（开始时间和结束时间共用一个日历组件）
  - 默认日期范围为选中月份第一天到最后一天
  - 点击日历图标展开日历，第一次点击选开始日期，第二次点击选结束日期
  - 区间中间日期显示浅粉色背景，开始/结束日期显示深粉色
  - 支持添加多个见习人员，每个人员独立配置姓名和日期范围
  - 新增 `/api/holiday/list` 接口，从内部服务器获取休假日数据
  - 日历中休假日在右上角显示"休"字红色标记
  - 新增后端API `/api/internship/sign` 用于生成签到表Word文档
  - 涉及的文件：`app.py`, `templates/cases_manage.html`
- **2026-04-02**: 立案查询页面添加"数据共享"功能：
  - 在立案查询页面添加"数据共享"按钮（位于"返回案件管理"旁边）
  - 支持三种查询类型：身份证信息、企业信息、机关单位信息
  - 企业查询支持按名称或统一社会信用代码查询
  - 新增后端API `/api/datashare/query` 统一处理数据共享查询
  - 前端弹窗展示查询结果，支持键值对表格形式展示
  - 涉及的文件：`app.py`, `templates/handle_query.html`, `modules/company_query.py`
- **2026-04-02**: 配置 HTTPS 服务，使用非特权端口 5443：
  - Nginx 监听 5443 端口提供 HTTPS 服务
  - Gunicorn 保持 5000 端口（HTTP）
